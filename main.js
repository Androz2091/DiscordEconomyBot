...
const quickdb = require('quick.db'); // npm install quick.db
var AsciiTable = require('ascii-table'); // npm install ascii-table
...
bot.on('message', async (message) => {
    ...
    */
    ...
    var isAdmin = config.administrators.includes(message.author.username+'#'+message.author.discriminator);
   ...
   if(command === 'cooldown'){
    ...
    // Send a success message
    message.reply('le cooldown de **'+member.user.username+'** pour la commande **'+cmd+'** a été réinistialisé !');
}
...
function printDate(pdate){
    ...
    return thedate = day + ' ' + monthNames[monthIndex] + ' ' + year + " à " + hour + "h" + minute;
}
